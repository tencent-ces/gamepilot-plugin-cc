...
import { createBrokerEndpoint, parseBrokerEndpoint } from "../plugins/gamepilot/scripts/lib/broker-endpoint.mjs";
...
test("createBrokerEndpoint uses named pipes on Windows", () => {
  ...
  assert.equal(endpoint, "pipe:\\\\.\\pipe\\cxc-12345-gamepilot-acp");
  assert.deepEqual(parseBrokerEndpoint(endpoint), {
    kind: "pipe",
    path: "\\\\.\\pipe\\cxc-12345-gamepilot-acp"
  });
});
