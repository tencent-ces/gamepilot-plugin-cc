...
import { listenOnRestrictedUnixSocket } from "../plugins/gamepilot/scripts/lib/socket-permissions.mjs";
...
test("listenOnRestrictedUnixSocket sets restrictive umask only while listening", () => {
  ...
  try {
    const onListening = () => {};
    listenOnRestrictedUnixSocket(server, "/tmp/gamepilot-acp.sock", onListening);

    assert.equal(observedPath, "/tmp/gamepilot-acp.sock");
...
test("listenOnRestrictedUnixSocket creates a real unix socket with mode 0o600", async (t) => {
  ...
  const sockPath = path.join(
    os.tmpdir(),
    `gamepilot-acp-int-${process.pid}-${Date.now()}.sock`
  );
