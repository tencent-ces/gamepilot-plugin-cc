...
/**
 * Stop review gate hook — optionally runs a GamePilot adversarial review of
 ...
 * 3. Sends the response to GamePilot for review via ACP
 * 4. Returns ALLOW or BLOCK based on GamePilot's assessment
 */

import fs from "node:fs";
import process from "node:process";

import { getGamePilotAvailability } from "./lib/gamepilot.mjs";
...
function runStopReview(cwd, input) {
  ...
  const result = runCommand("gpc", ["-p", prompt, "--output-format", "text", "--approval-mode", "plan"], {
    cwd,
  ...
  if (result.error || result.status !== 0) {
    return {
      ok: false,
      reason: `GamePilot review failed: ${result.stderr?.slice(0, 200) || "unknown error"}`
    };
  }
  ...
  return { ok: true, reason: `GamePilot response did not match expected format. Allowing. First line: ${firstLine.slice(0, 100)}` };
}
