/**
 * ACP (Agent Client Protocol) JSON-RPC client for communicating with `gpc --acp`.
 *
 * Three classes:
 * - AcpClientBase: Shared JSON-RPC logic (request/response matching, notifications, line parsing)
 * - SpawnedAcpClient: Spawns `gpc --acp` as a child process (direct mode)
 * - BrokerAcpClient: Connects to broker via Unix socket
 *
 * Factory:
 * - GamePilotAcpClient.connect(): Tries broker first, falls back to direct spawn
 */
...
export const BROKER_ENDPOINT_ENV = "GAMEPILOT_COMPANION_ACP_ENDPOINT";
...
class AcpClientBase {
  ...
  handleLine(line) {
    ...
    if (message.method === BROKER_DIAGNOSTIC_METHOD) {
      ...
      // the `gpc --acp` child — a forged notification on its stdout MUST
  ...
  async handshake() {
    const result = await this.request("initialize", {
      protocolVersion: 1,
      clientInfo: {
        name: PLUGIN_MANIFEST.name ?? "gamepilot-plugin-cc",
...
class SpawnedAcpClient extends AcpClientBase {
  ...
  sendMessage(message) {
    ...
    if (!stdin) {
      throw new Error("gpc --acp stdin is not available.");
    }
    stdin.write(line);
  }
}
...
export class GamePilotAcpClient {
  /**
   * Connect to a GamePilot ACP instance. Tries broker first, falls back to direct.
   *
   * @param {string} cwd
