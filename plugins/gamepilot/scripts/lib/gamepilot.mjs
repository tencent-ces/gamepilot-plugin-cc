/**
 * Core GamePilot functions — availability checks, auth status, running prompts,
 * reviews, and tasks through ACP.
 *
 * This mirrors the Codex plugin's codex.mjs but uses GamePilot's ACP protocol
 * (JSON-RPC 2.0 via `gpc --acp`) instead of Codex's app-server.
 */
...
function emitThinkingWarningIfNew(writer = (s) => process.stderr.write(s)) {
  ...
  writer(
    "Warning: --thinking is parsed but not delivered to the running GamePilot CLI. " +
    "Configure thinkingConfig at the model-alias level in your GamePilot settings.json " +
    "for a persistent setting. See " +
    "https://github.com/google-gamepilot/gamepilot-cli/blob/main/docs/cli/generation-settings.md\n"
  );
  thinkingWarned = true;
}
...
/**
 * Check GamePilot authentication status.
 ...
 */
export async function getGamePilotAuthStatus(cwd) {
  // Quick check: if GAMEPILOT_API_KEY is set, we're authenticated.
  if (process.env.RDEC_X_API_KEY) {
    return { authenticated: true, method: "api-key" };
  }

  // Try ACP: connect, inspect authMethods from initialize, then probe each.
  try {
    const client = await GamePilotAcpClient.connect(cwd, { disableBroker: true });
    try {
...
/**
 * Run a prompt through GamePilot ACP and capture the result.
 *
 * @param {string} cwd
 * @param {string} prompt
...
export async function runAcpPrompt(cwd, prompt, options = {}) {
  ...
  const client = await GamePilotAcpClient.connect(cwd, {
    env: options.env,
  ...
  try {
    ...
    if (options.thinking !== undefined) {
      ...
      // Delivery: upstream GamePilot CLI (0.38.x) does not accept a per-invocation
...
export function buildPersistentTaskThreadName(taskText) {
  ...
  return `gamepilot-task: ${truncated}`;
}
