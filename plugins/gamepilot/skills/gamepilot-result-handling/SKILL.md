...
- If the helper reports that setup or authentication is required, direct the user to `/gamepilot:setup` and do not improvise alternate auth flows.
- Do not fabricate results for incomplete or non-terminal jobs. If `/gamepilot:status` reports a job as still running, `quiet`, `possibly_stalled`, `rate_limited`, `auth_required`, `broker_unhealthy`, or `worker_missing`, preserve the reported health status, health message, and recommended action instead of inventing an answer. Direct the user to `/gamepilot:status`, `/gamepilot:result`, or `/gamepilot:cancel` as the recommended action suggests.
